        ...
        @if(isset($offers))
            @foreach($offers as $offer)
                ...
                @php
                    ...
                    // Application form details (only when linked)
                    $appCode    = $offer->application?->application_code ?? null;
                    $appDate    = $offer->application?->created_at       ?? null;
                    $appEmail   = $offer->application?->email ?? $offer->email ?? null;
                    $appPhone   = $offer->application?->phone ?? $offer->phone ?? null;

                    // Duration calculation using DateInterval for exact months + days
                    $joiningDate    = \Carbon\Carbon::parse($offer->joining_date);
                    $completionDate = \Carbon\Carbon::parse($offer->completion_date);
                    $interval       = $joiningDate->diff($completionDate);
                    $totalMonths    = ($interval->y * 12) + $interval->m;
                    $remainingDays  = $interval->d;
                    $totalDays      = $joiningDate->diffInDays($completionDate);

                    $monthWord = $totalMonths === 1 ? 'Month' : 'Months';
                    $dayWord   = $remainingDays === 1 ? 'Day' : 'Days';

                    if ($totalMonths > 0 && $remainingDays > 0) {
                        $durationLabel = "{$totalMonths} {$monthWord} {$remainingDays} {$dayWord}";
                    } elseif ($totalMonths > 0) {
                        $durationLabel = "{$totalMonths} {$monthWord}";
                    } else {
                        $totalDayWord  = $totalDays === 1 ? 'Day' : 'Days';
                        $durationLabel = "{$totalDays} {$totalDayWord}";
                    }
                @endphp
                ...
                <div class="recipient-info">
                    ...
                    @if($appCode)
                        <span style="font-size:12px; color:#555;">
                            Application Ref: <strong>{{ $appCode }}</strong>
                            @if($appDate)
                                &nbsp;|&nbsp; Submitted: {{ \Carbon\Carbon::parse($appDate)->format('d/m/Y') }}
                            @endif
                        </span><br>
                    @endif
                </div>
                ...
                <p>
                    ...
                    <strong>{{ $offer->duration_text ?? 'General' }} {{ $offer->internship_role ?? 'Full Stack Developer' }} Internship Program (Open-source Technology)</strong> at Techstrota.
                </p>
                ...
                <div class="details-container">
                    ...
                    <table class="details-table">
                        <tr>
                            ...
                            <td>{{ $offer->internship_position ?? 'Intern' }}</td>
                        </tr>
                        <tr>
